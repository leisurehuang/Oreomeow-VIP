...
RS2() {
    ql raw https://raw.githubusercontent.com/leisurehuang/qinglong_auto_tools/master/scripts_check_dependence.py
}
RS3() {
...
# 🍪Ninja
update_Ninja_normal() {
    cd /ql/data/ninja/backend && git checkout . && git pull
...
install_js_pkgs_initial() {
    if [ -d "/ql/data/scripts" ] && [ ! -f "/ql/data/scripts/package.bak.json" ]; then
        cd /ql/data/scripts || exit
        rm -rf node_modules
        rm -rf .pnpm-store
        ...
        install 1 "npm install -g package-merge" "$(echo "$(npm install -g package-merge && npm ls -g package-merge)" | grep -cE '(empty)|ERR')" &&
            ...
            node -e \
                "const merge = require('package-merge');
                 const fs = require('fs');
                 const dst = fs.readFileSync('/ql/data/repo/Oreomeow_checkinpanel_master/package.json');
                 const src = fs.readFileSync('/ql/data/scripts/package.bak.json');
                 fs.writeFile('/ql/data/scripts/package.json', merge(dst, src), function (err) {
                     if (err) {
                         console.log(err);
                     }
    ...
    if command -v cpm >/dev/null 2>&1; then
        ...
        if ! command -v cpm >/dev/null 2>&1; then
            if [ -f ./cpm ]; then
                ...
                cp -f /ql/data/repo/Oreomeow_checkinpanel_master/cpm ./ && chmod +x cpm && ./cpm --version
                if [ ! -f ./cpm ]; then
