...
update_config() {
    curl -sL https://raw.githubusercontent.com/leisurehuang/Oreomeow-VIP/main/Scripts/sh/Helpcode2.8/config_sample.sh >"$file_raw_config"
...
update_extra() {
    curl -sL https://raw.githubusercontent.com/leisurehuang/Oreomeow-VIP/main/Tasks/qlrepo/extra.sh >"$file_raw_extra"
    ...
    echo 'bash /ql/data/config/Update.sh' >>"$file_config_extra"
}

update_code() {
    curl -sL https://raw.githubusercontent.com/leisurehuang/Oreomeow-VIP/main/Scripts/sh/Helpcode2.8/code.sh >"$file_raw_code"
...
update_task_before() {
    curl -sL https://raw.githubusercontent.com/leisurehuang/Oreomeow-VIP/main/Scripts/sh/Helpcode2.8/task_before.sh >"$file_raw_task_before"
    mv -b "$file_raw_task_before" "$dir_config"
    sed -i "s/jd_moneyTree_heip/jd_moneyTree*/" "$file_config_task_before"
}

update_ninja() {
    cd /ql && pm2 delete ninja && rm -rf /ql/data/ninja
    git clone -b master https://github.com/China-Uncle/Waikiki_ninja ninja
    cd /ql/data/ninja/backend || exit
    pnpm install
    cp -rf /ql/data/config/.env .env
    pm2 start
    cp -rf /ql/data/config/sendNotify.js /ql/data/ninja/backend/sendNotify.js
}
